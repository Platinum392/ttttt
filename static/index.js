...
function gameInit() {
    createjs.Sound.registerSound({
        src: "./Script/music/err.mp3",
        id: "err"
    });
    createjs.Sound.registerSound({
        src: "./Script/music/end.mp3",
        id: "end"
    });
    createjs.Sound.registerSound({
        src: "./Script/music/tap.mp3",
        id: "tap"
    });
    gameRestart();
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '你被绍轩打坐牢了';
    if (score <= 99) return '还是对抗路不熟练啊';
    if (score <= 149) return '还差一点，让他继续坐牢';
    if (score <= 199) return '绍轩在牢里出不来了';
    return '您就是坐牢天王？';
}
